...
class TransactionDataModuleV2(pl.LightningDataModule):
    ...
    def __init__(self,
        ...
        if self.is_predicting:
            ...
            # Corrected user mapping section:
            # self.user_map is already in the correct {name: code} format from fitted_user_map.
            # The transactions_df_ref['company_name'] (copied to self.transactions_df['company_name'])
            # should have been cleaned (NaNs handled, string type) in train_new.py 
            # before final_user_map (which becomes self.user_map here) was created.

            company_names_in_df = self.transactions_df['company_name']

            # Map using the provided self.user_map.
            # Ensure self.user_map is not None before attempting to map
            if self.user_map is not None:
                mapped_user_ids = company_names_in_df.map(self.user_map)

                if mapped_user_ids.isnull().any():
                    unmapped_count = mapped_user_ids.isnull().sum()
                    print(f"[CRITICAL WARN] TransactionDataModuleV2 (__init__): {unmapped_count} 'company_name' values in the DataFrame " \
                          f"did NOT exist as keys in the provided 'fitted_user_map' (self.user_map). These will become -1.")
                    # sample_unmapped_names = company_names_in_df[mapped_user_ids.isnull()].unique()[:5]
                    # print(f"[DEBUG] TransactionDataModuleV2: Sample unmapped company names: {list(sample_unmapped_names)}")
                    # print(f"[DEBUG] TransactionDataModuleV2: Provided user_map has {len(self.user_map)} entries. First 5 keys: {list(self.user_map.keys())[:5]}")

                # Overwrite user_id_code with the new mapping.
                # astype(int) will fail if NaNs are still present, so fillna(-1) is essential here if mapping can fail.
                self.transactions_df['user_id_code'] = mapped_user_ids.fillna(-1).astype(int)
            else:
                # This case should ideally not be reached if train_new.py always provides a valid final_user_map
                print("[CRITICAL ERROR] TransactionDataModuleV2 (__init__): 'fitted_user_map' (self.user_map) is None. Cannot map user IDs. Defaulting user_id_code to -1.")
                self.transactions_df['user_id_code'] = -1
            ...
            # self.transactions_df['user_id_code'] = self.transactions_df['company_name'].map(map_user_str_to_int).fillna(-1).infer_objects(copy=False).astype(int) # Old incorrect line
            
            self.num_global_classes = len(self.category_id_map)
            self.num_users = len(self.user_map) if self.user_map is not None else 0 # num_users is derived from the provided map
            
            # This warning will now correctly reflect any -1s generated by the mapping above.
            if (self.transactions_df['user_id_code'] == -1).any():
                print(f"[WARN] TransactionDataModuleV2 (__init__): Found {(self.transactions_df['user_id_code'] == -1).sum()} 'user_id_code' values of -1 " \
                      f"after re-mapping with the provided 'fitted_user_map'.")
        else:
